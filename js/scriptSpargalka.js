...
//TIMEOUT
const btn = document.querySelector('.btn');        // Получаем элемент с классом 'btn' и сохраняем его в переменную 'btn'
const stopBtn = document.querySelector('.stop-btn'); // Получаем элемент с классом 'stop-btn' для остановки таймера
let timerId;                                        // Объявляем переменную 'timerId' для хранения идентификатора таймера

btn.addEventListener('click', () => {               // Добавляем обработчик события 'click' к элементу 'btn'
	timerId = setInterval(logger, 2000);            // Запускаем выполнение функции 'logger' каждые 2000 мс и сохраняем идентификатор таймера
});

stopBtn.addEventListener('click', () => {           // Добавляем обработчик события 'click' к элементу 'stop-btn'
	clearInterval(timerId);                         // Останавливаем выполнение таймера
});

function logger() {                                 // Объявляем функцию 'logger'
	console.log('text');                            // Выводим строку 'text' в консоль
}

//Animaciya
function myAnimation() {
	const elem = document.querySelector('.box'); // Находим элемент с классом 'box'
	let pos = 0; // Начальная позиция
	const id = setInterval(frame, 10); // setInterval(frame, 10): Запускает функцию frame каждые 10 миллисекунд.

	function frame() {
		if (pos == 300) { // Проверяем, достигла ли позиция значения 300
			clearInterval(id); // Останавливаем интервал
		} else {
			pos++; // Увеличиваем позицию
			elem.style.top = pos + 'px'; // Изменяем свойство 'top' элемента
			elem.style.left = pos + 'px'; // Изменяем свойство 'left' элемента
		}
	}
}

const btn = document.querySelector('.btn'); // Находим элемент с классом 'btn'
btn.addEventListener('click', myAnimation); // Добавляем обработчик события 'click' к кнопке, чтобы запускать анимацию при клике

//Rabota s datami
const now = new Date();
console.log(now);

const now = Date('2020-05-01'); //peredaet datu
console.log(now);

const now = new Date(0); // vivedet 1970 god
console.log(now);

const now = new Date();
console.log(now.getFullYear());//vivedet ttekushiy god
console.log(now.getMonth());//mesyac
console.log(now.getDate());//den
console.log(now.getDay());//den nedeli
console.log(now.getUTCHours());//po grinvichu
console.log(now.setHours(40));//izmenit
...
document.addEventListener('DOMContentLoaded', function () {  // Ждем загрузки DOM, чтобы все элементы были доступны
	...
	//TIMER
	const deadline = '2026-08-11';                               // Устанавливаем дату дедлайна

	function getTimeRemaining(endtime) {                         // Функция для вычисления оставшегося времени до дедлайна
		const t = Date.parse(endtime) - Date.parse(new Date()),  // Вычисляем разницу во времени между дедлайном и текущим временем
			days = Math.floor(t / (1000 * 60 * 60 * 24)),        // Переводим оставшееся время в дни
			hours = Math.floor((t / (1000 * 60 * 60)) % 24),     // Переводим оставшееся время в часы
			minutes = Math.floor((t / 1000 / 60) % 60),          // Переводим оставшееся время в минуты
			seconds = Math.floor((t / 1000) % 60);               // Переводим оставшееся время в секунды
		return {                                                 // Возвращаем объект с вычисленным временем
	...
	function getZero(num) {                                      // Подставляем нолик вперед
		if (num >= 0 && num < 10) {
			return `0${num}`;
		} else {
			return num;
	...
	function setClock(selector, endtime) {                       // Функция для установки таймера на странице
		const timer = document.querySelector(selector),          // Получаем элемент таймера по селектору
			days = timer.querySelector('#days'),                 // Получаем элемент для отображения дней
			hours = timer.querySelector('#hours'),               // Получаем элемент для отображения часов
			minutes = timer.querySelector('#minutes'),           // Получаем элемент для отображения минут
			seconds = timer.querySelector('#seconds'),           // Получаем элемент для отображения секунд
			timeInterval = setInterval(updateClock, 1000);       // Устанавливаем интервал для обновления таймера каждую секунду
		updateClock();                                           // Вызываем чтоб сразу все обновилось

		function updateClock() {                                 // Функция для обновления значений таймера
			const t = getTimeRemaining(endtime);                 // Получаем оставшееся время до дедлайна
			days.innerHTML = getZero(t.days);                             // Обновляем значение дней в таймере
			hours.innerHTML = getZero(t.hours);                           // Обновляем значение часов в таймере
			minutes.innerHTML = getZero(t.minutes);                       // Обновляем значение минут в таймере
			seconds.innerHTML = getZero(t.seconds);                       // Обновляем значение секунд в таймере
			if (t.total < 0) {                                   // Проверяем, если время истекло
				clearInterval(timeInterval);                     // Останавливаем интервал обновления таймера
			}
		}
	}
	setClock('.timer', deadline);                                // Устанавливаем таймер, вызывая функцию setClock с селектором таймера и дедлайном

	// Modal
	const modalTrigger = document.querySelectorAll('[data-modal]'),  // Находим все элементы, которые будут открывать модальное окно
		modal = document.querySelector('.modal'),                   // Находим само модальное окно
		modalCloseBtn = document.querySelector('[data-close]');     // Находим кнопку закрытия модального окна

	modalTrigger.forEach(btn => {                                   // Для каждого элемента, открывающего модальное окно
		btn.addEventListener('click', openModal);                   // Добавляем обработчик события 'click', который вызывает функцию openModal
	});

	function closeModal() {                                         // Функция для закрытия модального окна
		modal.classList.add('hide');                                // Добавляем класс 'hide' для скрытия модального окна
		modal.classList.remove('show');                             // Убираем класс 'show' для скрытия модального окна
		document.body.style.overflow = '';                          // Включаем прокрутку страницы при закрытом модальном окне
	}

	function openModal() {                                          // Функция для открытия модального окна
		modal.classList.add('show');                                // Добавляем класс 'show' для отображения модального окна
		modal.classList.remove('hide');                             // Убираем класс 'hide' для отображения модального окна
		document.body.style.overflow = 'hidden';                    // Отключаем прокрутку страницы при открытом модальном окне
		clearInterval(modalTimerId);                                // Останавливаем таймер, если он был запущен
	}

	modalCloseBtn.addEventListener('click', closeModal);            // Добавляем обработчик события 'click' для кнопки закрытия модального окна, который вызывает функцию closeModal

	modal.addEventListener('click', (e) => {                        // Добавляем обработчик события 'click' для самого модального окна
		if (e.target === modal) {                                   // Проверяем, что клик был по фону модального окна, а не по его содержимому
			closeModal();                                           // Закрываем модальное окно
		}
	});

	document.addEventListener('keydown', (e) => {                   // Добавляем обработчик события 'keydown' для документа
		if (e.code === "Escape" && modal.classList.contains('show')) { // Проверяем, что нажата клавиша Escape и модальное окно открыто
			closeModal();                                           // Закрываем модальное окно
		}
	});

	const modalTimerId = setTimeout(openModal, 3000);               // Устанавливаем таймер на 3 секунды для автоматического открытия модального окна

	function showModalByScroll() {                                  // Функция для открытия модального окна при прокрутке до конца страницы
		if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight) { // Проверяем, что пользователь прокрутил страницу до конца
			openModal();                                            // Открываем модальное окно
			window.removeEventListener('scroll', showModalByScroll); // Удаляем обработчик события 'scroll' после открытия модального окна
		}
	}
	window.addEventListener('scroll', showModalByScroll);           // Добавляем обработчик события 'scroll' для окна, который вызывает функцию showModalByScroll

});

'use strict';                                                    // Включаем строгий режим
const box = document.querySelector('.box');                      // Находим элемент с классом 'box'
let observer = new MutationObserver(mutationRecords => {         // Создаем объект MutationObserver, который будет следить за изменениями в DOM
	console.log(mutationRecords);                                // Выводим записи мутаций в консоль
});
observer.observe(box, {                                          // Настраиваем наблюдатель для элемента 'box'
	childList: true                                              // Следим за изменениями в списке дочерних элементов
});
observer.disconnect();

// Функции-конструкторы
function User(name, id) {                                       // Определяем функцию-конструктор User
	this.name = name;                                           // Устанавливаем свойство name
	this.id = id;                                               // Устанавливаем свойство id
	this.human = true;                                          // Устанавливаем свойство human, значение true
	this.hello = function () {                                  // Определяем метод hello
		console.log(`Hello ${this.name}`);                      // Выводим приветственное сообщение с именем пользователя
	};
}
User.prototype.exit = function () {                             // Добавляем метод exit в прототип User
	console.log(`Пользователь ${this.name} ушел`);              // Выводим сообщение о том, что пользователь ушел
};
const ivan = new User('Ivan', 28);                              // Создаем новый объект ivan с помощью функции-конструктора User
const alex = new User('Alex', 20);                              // Создаем новый объект alex с помощью функции-конструктора User
ivan.exit();                                                    // Вызываем метод exit для объекта ivan
ivan.hello();                                                   // Вызываем метод hello для объекта ivan
